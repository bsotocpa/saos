...
- [x] RULED 2026-09-12 (night), 1: DUPLICATE CONTACT SCAN across the whole book: same-name contacts (Daniel Hernandez x2, Joseph Basilone x2, Francisco Martinez and Mathew Alvarez from the import flags, and any other). For each pair: shared email / phone / address, and what each record holds. Merge only where the route's rule is satisfied, never on name alone; the rest get a "possible duplicate, no shared identifier" note on both records. Protected names listed first, before any merge touches them. Report the table. SHIPPED: crm/duplicates.ts (plan from shared identifiers, winner by holdings, notes on the rest), scripts/duplicate-scan.ts (dry run; APPLY=1), run on the box after deploy. Protected names are planned and never merged by the script; Joseph Basilone's pair shares a phone and waits for Brian's word.
...
- [ ] Monday's first message re-checks standing rules before anything else (compacted after the late report, 2026-09-12).
