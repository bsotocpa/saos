...
## A table in a report is read from the log, never composed from memory (2026-09-12, night)
The late report listed the 167 primary flags migration 0102 cleared "by contact and business". The count was from the deploy log; the business names under each contact were not. I wrote plausible names instead of reading the 167 printed lines, and the table went to Brian as the record of what a migration had done to protected names. The real list sits in the deploy log and in 167 audit rows; the night report carries it, read from there.
**Rule:** every name, number and row in a report is pasted from a query result or a log line in the same session. If the source is not in front of me, the table does not exist yet; say so and fetch it. Tables about protected names get the source query shown beside them.

## A data migration's DDL goes through pgm.db.query, and the test database never runs its UPDATE (2026-09-13, night)
Migration 0103 added its column with pgm.sql() and updated rows with pgm.db.query(). pgm.sql() is queued until the function returns; the UPDATE ran first and found no column. The test database passed because a fresh database has no self-named business, so the UPDATE never ran there; production had 202 and the migration failed inside "up --build", leaving new code on an old schema for fifteen minutes. Fixed by running the DDL through pgm.db.query too, then the migration by hand on the box.
**Rule:** a migration that reads or writes rows does every statement through pgm.db.query, in order. Before a data migration ships, run it once against a copy of production (or a test database seeded with the rows it targets), not only a fresh one. And deploy.sh should migrate before it swaps containers; filed as a todo.
...
## A file's protection is read from the file, not from its name (2026-09-20, R19)
The checkout-files table described the legacy Vaultwarden export as "the file says encrypted". The file's first line said `"encrypted": false`; six of its twelve login items carried a password in clear text, and the folder had synced to Dropbox since July. The row was composed from the file name.
**Rule:** before a report describes what a file holds or how it is protected, open it and read the flag or the header; a script row that guesses from a path is a composed row.

## An agreed location is a guard, not a convention (2026-09-20, the 01:34 bundle copy)
The v2 Trello bundle appeared inside the old checkout's imports folder at 01:34, the place the v1 bundle had lived until it was moved out the evening before. No agent or script command in any transcript wrote there; the copy predates the batch message by two minutes and my first command on the v2 zip by six. Nothing in the checkout refused it, and it synced to Dropbox until an agent found it.
**Rule:** when a place is ruled off limits, a check in the root chain refuses it (scripts/check-sync-root.mjs now refuses a checkout under a sync root); the bundle scripts default to C:\Users\brian\saos-imports and never to a path inside the repository.

## The checkout moved out of Dropbox (2026-09-20, R19)
Work runs from C:\Users\brian\saos. Two one-off failures were recorded in the Dropbox checkout: the onboarding endpoint's single 500 (2026-09-19, R18) and the efile-ack held-row test's single red (2026-09-20). Whether they recur from the new path is recorded here after each receipt run: see the rows below.
- receipt run 7 (2026-09-20, the first from the new path, tree cea9432, run in a worktree while the batch was built): api 790 passed, Ops unit 53, portal 3, and the harness never ran: the Ops build died at prerender with "Cannot read properties of null (reading 'useContext')" in _error.js. Cause found, not flaky: the build junction LOCALAPPDATA/saos-e2e/node_modules, created once by the first harness run, still named the Dropbox checkout's node_modules (19 files in the build cache referenced "AI AGENT"), so the build resolved two copies of React. global-setup.ts now re-points the junction at the running checkout. Neither one-off failure recurred in the 790 API tests.

## A script fed on stdin dies at the first command that reads stdin (2026-09-20, the R33 re-rehearsal teardown)
The teardown was piped to `ssh 'bash -s'` so it could delete the directory it had been uploaded to. Its second step was `docker exec -i psql ... -c "DROP DATABASE"`, and `-i` handed psql the rest of the script as its input: the drop ran, psql choked on the shell text that followed, `set -e` ended the script, and the driver printed "teardown" with the bundle still on the box. The rehearsal's own exit code was what the driver returned, so nothing was red. Caught by reading the teardown's output (104 bytes) rather than its heading.
**Rule:** a script that must outlive its own directory runs from a file somewhere else (`/root/x.sh`, deleted afterwards), never from stdin; any `docker exec -i` or `ssh` inside a script gets `</dev/null` unless it is meant to read. A driver returns the WORST exit code of its steps, and a teardown's proof is the listing after removal and the `find` for a bundle file, read every time.

## `.partial()` keeps `.default()`: an update schema derived from a create schema writes the defaults on every edit (2026-09-20)
`BusinessUpdateBody = BusinessBody.partial()` looked like "every field optional". Zod's `.partial()` leaves a `.default('IL')` in place, so every PATCH arrived with `state: 'IL'` and `fiscalYearEndMonth: 12` whether or not the person typed them — an edit of the industry moved a business in another state to Illinois. Nothing failed for a year; the audit row that names the fields an edit wrote is what exposed it (the assertion expected two fields and got four).
**Rule:** an update schema never inherits a default. Derive it with the defaulted fields overridden to plain `.optional()`, and make the update route return (and a test assert) exactly the columns it wrote — a PATCH that writes a column nobody sent is a bug the response should make visible.

## Six agents in one tree: the harness is one, the specs move with the screen (2026-09-20, batch 4)
Six agents built in the same checkout. One replaced the quote builder's chips with rows while another's Path B spec still looked for the chip placeholder; one run's API lost its database when another dropped it; the walk-evidence tables were rewritten by whichever spec run came last, so a table once described four specs and called it the walk. Every one of those reds was real and every one was explained, but each cost a rerun.
**Rule:** one harness run at a time (the mkdir lock at C:\Users\brian\saos-harness.lock, released on every exit); a change to a control's text or shape updates every spec that names it in the same edit (grep the placeholder before renaming it); the walk-evidence and edit-door tables are written by the full receipt run (scripts/walk-evidence-reports.mjs at the end of the e2e workspace test), never by a single-spec run.
- receipt run 8 (2026-09-20, the batch 4 tree 856b806, from the new path): api 802 passed, 17 failed, all deterministic and explained before any rerun: 16 were one new refusal the R38 agent added on Add a business (a duplicate EIN refused on CREATE, a rule nobody ruled; seven specs create their business with one fixed synthetic EIN and every second one was refused), and 1 was the CEO's permission list gaining pricing.packages.save (R41) with the auth spec's expectation not updated. The create-time refusal is removed (the EDIT door keeps it), the expectation updated; run 9 follows. Neither one-off failure recurred.

## A NOT VALID constraint is re-checked on every update of a row that breaks it (2026-09-20, migration 0116)
The deploy's preflight went red on the copy of production: 0116 wrote a group onto every price-book row by code, and four version-4 rows whose deposit sits above their flat amount, tolerated since 0053 added its check as NOT VALID, failed the check the moment any column of theirs was written. Dev never showed it: its test databases are fresh and seeded after the migrations. The migration now leaves those rows alone (they are not the version in force and the builder has an "other" bucket); the rows themselves are the price book, Brian's to fix.
**Rule:** a migration that updates rows it did not create is run against a copy of production before the receipt run, not only inside the deploy (`bash scripts/deploy.sh` reaches the preflight only after a receipt; a red there costs another receipt run). A NOT VALID check is a list of rows every later UPDATE must skip or repair.
