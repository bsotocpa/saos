/*
 * "NO FORMATION DATE ON RECORD" — what it blocks depends on the state's shape.
 *
 * Found while production-verifying the Florida work: the book holds eight Florida businesses and
 * not one of them has a formation date anywhere. Under the old code every one would have enrolled
 * into a compliance row with a NULL due date — invisible rather than pending, because the status
 * sweep skips null dates and both reminder loops load by exact due date. Tracked-looking, and no
 * reminder ever coming.
 *
 * The shape decides the answer, which is the first time modelling it as a shape paid for itself:
 *
 *   · ILLINOIS blocks — the formation date IS the deadline, so nothing derives. Enrolment now
 *     creates a "Find the formation date" task, so the gap is owned work rather than a blank row.
 *   · FLORIDA does not — 1 May is 1 May. The date derives with no formation date at all; only
 *     the first-year skip is lost, and that errs toward the EARLIER date on purpose. An early
 *     report is a wasted filing; a late one is $400 Sunbiz has no provision to waive.
 *
 * Laura's page gains a section saying exactly that, because "it depends on the state" is useless
 * to the person holding the file unless it says which states and why.
 *
 * Same mechanism as 0070/0071/0073/0074: the seeder is ON CONFLICT DO NOTHING, so an edited seed
 * reaches only fresh databases. 0074 already ran and will not run again — this carries the newer
 * text. Scoped to version = 1, the un-hand-edited text.
 *
 * Body generated FROM the seed so the two agree today, then frozen. `down` is empty: restoring a
 * page that does not warn about the silent row would put worse guidance in front of the person
 * doing the work.
 */
...
exports.up = (pgm) => {
  ...
  pgm.sql(`
    ...
    BEGIN
      ...
      IF v = 1 THEN
        RAISE WARNING '0075: laura-annual-report updated — the no-formation-date section is in';
      ELSIF v IS NULL THEN
        RAISE WARNING '0075: laura-annual-report absent — a fresh database seeds the new text directly';
      ELSE
        RAISE WARNING '0075: laura-annual-report is at version % — hand-edited, LEFT ALONE. Re-apply the no-formation-date section by hand if missing.', v;
      END IF;
    END
    $report$;
  `);
