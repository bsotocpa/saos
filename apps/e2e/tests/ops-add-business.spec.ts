 ...
 * the CEO, proves the refusal renders beside the control that caused it (a formation date after
 * today), and proves one role that cannot: Laura (va_entity) holds no contacts.write, so the
 * button is not on her page and the route refuses her.
 */
...
test.describe('Add a business on the client page', () => {
  ...
  test('role proof: Laura (va_entity) has no Add a business button and the route refuses her', async ({ page }, testInfo) => {
    ...
    const status = await page.evaluate(async (id) => {
      const r = await fetch(`/api/contacts/${id}/businesses`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: 'HARNESS-LAURA-REFUSED LLC', entityType: 'llc', state: 'IL' }) });
      return r.status;
    }, fixtures.contactId);
    expect(status, 'the route refuses a role without contacts.write').toBe(403);
    testInfo.annotations.push({ type: 'walk-step', description: `A1|role proof: va_entity sees no button, POST refused 403|${ROLES}|tap` });
  });
});
